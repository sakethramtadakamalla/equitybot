...
def fetch_company_info(symbol):
    try:
        ...
        info = ticker.info
        # Check for empty info dict, a sign of an invalid ticker or failed fetch
        ...
        # Re-raise a generic error that the main function will catch
        raise ValueError(f"Could not retrieve critical data for {symbol}. The ticker may be invalid or the data source is temporarily unavailable.")

# ... (All other helper functions like fetch_logo, fetch_price, compute_technical_indicators, etc. remain the same)
...
def generate_recommendation(fundamentals, tech_analysis):
    score = 0
    roe = fundamentals.get('ROE'); pe = fundamentals.get('Trailing PE'); de = fundamentals.get('Debt to Equity')
...
def interpret_technical(df):
    ...
    bb_upper = get_scalar(latest.get('bb_upper')); macd_line = get_scalar(latest.get('macd_line')); macd_signal = get_scalar(latest.get('macd_signal'))
...
class ReportPDF:
    # ... (The entire ReportPDF class remains the same)
    ...
    def create_pdf(self, price_df, news, peer_df, tech_analysis, recommendation, key_highlights):
        ...
        self.story.append(Spacer(1, 2*inch))
        self.story.append(Paragraph(f"Report Generated: {datetime.now().strftime('%B %d, %Y')}", self.styles['Normal']))
        self.story.append(PageBreak())

        # Page 2: Overview and Highlights
        ...
        if financials is not None and not financials.empty and balance_sheet is not None and not balance_sheet.empty:
            ...
            for item in fin_items:
                if item in all_fins.index:
                    row = [item] + [f"₹{val/1e7:,.0f} Cr" for val in all_fins.loc[item].iloc[:3]]
                    fin_data.append(row)
            self.story.append(Table(fin_data, style=self.navy_blue_header_style))
        else:
            self.story.append(Paragraph("Annual financial data not available.", self.styles['Normal']))
        self.story.append(Spacer(1, 12))
        
        self.story.append(Paragraph("Quarterly Performance", self.styles['h3']))
        ...
        if q_financials is not None and not q_financials.empty:
            ...
            for item in q_items:
                if item in q_financials.index:
                    row = [item] + [f"₹{val/1e7:,.0f} Cr" for val in q_financials.loc[item]]
                    q_data.append(row)
        ...
        if news:
            ...
            self.story.append(Paragraph("Recent news could not be fetched at this time.", self.styles['Normal']))
...
def generate():
    try:
        ...
        user_friendly_error = str(e).replace("Could not retrieve critical data", "The data source is currently busy or unavailable for")
        return jsonify({'error': f"{user_friendly_error}. Please wait 30 seconds and try again."}), 400
    except Exception as e:
